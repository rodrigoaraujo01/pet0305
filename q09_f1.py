...
def normalize_array(array):
    norm = [(i - min(array))/(max(array) - min(array)) for i in array]    
    return norm


class PatternGenerator(object):
    def __init__(self, n_samples, future, debug=False):
        self.debug = debug
        self.n_samples = n_samples
        X = [i/10 for i in range(100)]  # i de 0 a 10
        f_X = [i/10 for i in range(future, 100+future)]  # i de 0 a 10
        y = [math.log(1 + math.cos(i + math.sin(i) * math.sin(i))) for i in X]
        y = normalize_array(y)
        # Future 1st value of y
        f_y = [math.log(1 + math.cos(i + math.sin(i) * math.sin(i))) for i in f_X]
        f_y = normalize_array(f_y)
        self.base_X = X
        self.future_X = f_X
        self.base_y = y
        self.future_y = f_y
        Z = [(i,j) for i,j in zip(self.base_X, self.future_y)]
        random.shuffle(Z)
        X = []
        y = []
        for i, j in Z:
    ...
    def better_plot_data(self, clf, lbl):
        fig = plt.figure()
        fig.suptitle(lbl)
        ...
        ax1.scatter(self.base_X, self.base_y, label='curva real', alpha=0.5)
...
def main():
    print("Começou")
    debug = True
    pg = PatternGenerator(n_samples=150, future=3, debug=debug)

    narx = NARXNetwork(pg.X, pg.y, [10,10], 0, 5, 500)
    narx.fit()
    print(narx.accuracy())

    # pg.better_plot_data([nn for nn in neural_networks], labels)
    pg.better_plot_data(narx, '')
    # pg.better_plot_data([], [])
